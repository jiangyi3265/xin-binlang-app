<template>
	<view class="pg bl-paper-tex">
		...
		<view class="bl-wrap body">
			<view class="notice"><bl-icon name="lock" :size="26" color="#2C7256" :weight="1.9" /><text>其他销售创建的门店在服务端已隔离，不会出现在这里。</text></view>
			...
			<view v-for="item in list" :key="item.id" class="store bl-card">
				...
				<view class="address"><bl-icon name="map-pin" :size="25" color="#8B9A92" :weight="1.8" /><text>{{ item.addr }}</text></view>
				<view class="metrics">
				...
				<view class="actions">
					<view @tap="accounts(item)"><bl-icon name="users" :size="24" color="#0E3B2E" :weight="1.9" /><text>账号</text></view>
					<view @tap="edit(item)"><bl-icon name="edit" :size="24" color="#0E3B2E" :weight="1.9" /><text>编辑</text></view>
...
<style lang="scss" scoped>
	.pg{min-height:100vh;padding-bottom:50rpx}.logout{padding:10rpx 18rpx;color:#FFF6E6;font-size:22rpx}.hero{padding:24rpx $bl-pad 30rpx;background:$bl-green;display:flex;align-items:center;justify-content:space-between;color:#FFF6E6}.hello{display:block;font-size:36rpx;font-weight:800}.scope{display:block;margin-top:4rpx;color:rgba(255,246,230,.6);font-size:21rpx}.count{text-align:right}.count text:first-child{display:block;font-size:42rpx;font-weight:800;color:$bl-gold-3}.count text:last-child{font-size:20rpx;color:rgba(255,246,230,.58)}.body{padding-top:24rpx}.notice{display:flex;align-items:flex-start;gap:10rpx;padding:18rpx 20rpx;border-radius:$bl-r-sm;background:$bl-green-lt;color:$bl-green-2;font-size:21rpx}.add{height:84rpx;margin-top:18rpx;gap:10rpx;font-size:27rpx}.store{margin-top:20rpx;padding:24rpx}.store-head{display:flex;align-items:flex-start;justify-content:space-between}.name{display:block;font-size:31rpx;font-weight:800;color:$bl-ink}.id{display:block;margin-top:2rpx;color:$bl-ink-4;font-size:20rpx}.address{display:flex;align-items:flex-start;gap:8rpx;margin-top:15rpx;color:$bl-ink-2;font-size:23rpx}.address text{flex:1}.metrics{display:grid;grid-template-columns:repeat(3,1fr);margin-top:22rpx;padding:18rpx 0;background:$bl-paper-2;border-radius:$bl-r-sm}.metrics view{text-align:center;border-right:1rpx solid $bl-line}.metrics view:last-child{border-right:0}.metrics text:first-child{display:block;color:$bl-green;font-size:30rpx;font-weight:800}.metrics text:last-child{font-size:19rpx;color:$bl-ink-3}.actions{display:grid;grid-template-columns:repeat(3,1fr);gap:12rpx;margin-top:18rpx}.actions view{height:62rpx;display:flex;align-items:center;justify-content:center;gap:7rpx;border:1rpx solid $bl-line;border-radius:$bl-r-sm;color:$bl-green;font-size:22rpx}.actions .danger{color:$bl-red}.loading{text-align:center;padding:60rpx;color:$bl-ink-3;font-size:23rpx}
</style>
