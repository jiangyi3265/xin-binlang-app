...
function runtime() {
  const cache = new Map(), storage = new Map(), requests = [], pages = []
  ...
  const uni = {
    ...
    request(options) {
      ...
      const data = path === '/api/customer/auth/wechat' ? { token: 'unit-test-session-' + loginCount, user: { id: 'test-customer' } }
  ...
  function load(file) {
    ...
    runInNewContext(code, { module, exports: module.exports, uni, getCurrentPages: () => pages, setTimeout, clearTimeout,
  ...
  return { store, api, redeem, login, requests, pages,
