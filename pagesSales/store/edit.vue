	...
	<view class="pg bl-paper-tex">
		...
		<view class="bl-wrap body">
			...
			<view class="form bl-card">
				<label><text>门店全称</text><input v-model="form.name" placeholder="例如：金榔记雨花亭旗舰店" maxlength="120" /></label>
				...
				<view class="map-btn" @tap="chooseLocation"><bl-icon name="map-pin" :size="28" color="#0E3B2E" :weight="1.9" /><text>从微信地图选择位置</text></view>
				<view class="coords">
				...
				<view class="status"><view><text>门店状态</text><text>{{ form.status === 'active' ? '启用后可供顾客选择' : '停用后不在小程序展示' }}</text></view><switch color="#0E3B2E" :checked="form.status === 'active'" @change="form.status=$event.detail.value?'active':'disabled'" /></view>
			</view>
