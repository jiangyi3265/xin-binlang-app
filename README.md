...
活动包装支持 30 元棕金和 50 元深蓝独立展示；输入兑换码后依据服务端批次识别。未配置图片的现金奖品自动展示红包图案。其他翻牌卡片展示已配置的其他奖项，优先当前奖池，再补充其他公开且启用奖池的奖品，排除本次中奖奖品并去重；实际开奖仍仅使用兑换码所属奖池。配置方法见关联后台仓库的 `docs/包装与奖项设置.md`。

1.0.8 优先使用预览接口的 `displayPrizes`。连接尚未升级的后端时，额外读取现有公开 `/public/prizes` 补充普通展示列表，可继续验证翻牌显示修复；“仅展示（不参与开奖）”奖品需先部署对应后端迁移。该补充查询不开奖，也不改变库存。

预览奖品可以带 `displayOnly`、`showcaseWeight` 等展示元数据；小程序只将它们转换成“奖品展示”卡片，不把展示奖品 ID 或权重提交给 `/customer/draw`。实际奖励始终以服务器返回的兑奖记录为准。
